            ...
            if event.type == pygame.QUIT:
                ...
                for i, niv in enumerate(niveaux):
                    ...
                    if (x - nx) ** 2 + (y - ny) ** 2 <= 15 ** 2:  # Clic dans le cercle
                        if niv["actif"]:
                            # --- Si clic sur le premier niveau (index 0) ---
                            if i == 0:
                                arreter_musique()  # stop la musique du menu / carte
                                jouer_musique("level1_music.mp3")  # lance la musique du niveau 1
